...
export default function DiscoverPage() {
  ...
  const filtered = (products || []).filter((p) => {
    const matchesCategory =
      activeCategory === 'All' || TYPE_LABELS[p.type] === activeCategory || p.title?.toLowerCase().includes(activeCategory.toLowerCase())
    const matchesSearch =
      searchQuery === '' ||
      ...
      <main className="pt-16">
        <section className="relative py-20 md:py-28 flex flex-col items-center text-center container-site overflow-visible">
          <div className="hidden lg:block absolute left-[-40px] top-32 animate-float pointer-events-none select-none">
            <div className="w-16 h-16 rounded-full bg-creator-pink border-2 border-primary flex items-center justify-center text-2xl font-bold rotate-[-12deg]">D</div>
          </div>
          <div className="hidden lg:block absolute right-[-40px] top-52 animate-float-delayed pointer-events-none select-none">
            <div className="w-20 h-20 rounded-full bg-creator-pink border-2 border-primary flex items-center justify-center text-3xl font-bold rotate-[8deg]">D</div>
          </div>
          ...
          <p className="text-[18px] md:text-subheading text-on-surface-variant max-w-[560px] mb-8 leading-relaxed">
            ...
            USDC. Your content protected by encryption. Nobody can deplatform you.
          </p>

          <div className="flex flex-col sm:flex-row gap-3 mb-12">
            <Link to="/dashboard" className="btn-primary">Start selling for free</Link>
        ...
        <section className="container-site mb-16">
          <div className="relative mb-4">
            <FiSearch className="absolute left-5 top-1/2 -translate-y-1/2 text-on-surface-variant" size={18} />
            <input
              type="text"
        ...
        <section id="products" className="container-site mb-20">
          <h2 className="text-subheading font-semibold text-primary mb-8">Featured Products</h2>
        ...
        <section className="container-site mb-20">
          <div className="bg-primary text-white rounded-lg p-12 flex flex-col md:flex-row items-center justify-between gap-8">
            <div>
              <h2 className="text-headline-lg font-bold mb-2">Start selling your digital products</h2>
              <p className="text-white/70 text-body-md max-w-[500px]">
                Join thousands of creators earning in USDC. No middleman. No deplatforming. Permanent ownership on-chain.
              </p>
            </div>
            <Link to="/dashboard" className="flex items-center gap-2 bg-white text-primary font-semibold px-8 py-3 rounded-full whitespace-nowrap hover:bg-marketplace-gray transition-all">
